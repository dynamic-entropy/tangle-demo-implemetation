...
def verify_signature(sender_public_key, signature, transaction):
    ...
    if initial:
        for i in range(initial_count):
            ...
            transaction = transaction_block(
                ...
                approved_tx=tips,
                nonces=[proof_of_work(previous_hashes, tangle.transactions[tips[0]]),
                        proof_of_work(previous_hashes, tangle.transactions[tips[1]])],
...
class Tangle:
    ...
    def find_tips(self, algo='weighted_random_walk'):
        ...
        if algo=='recently_added':
            return list(random.sample(set(list(self.transactions.keys())[-5:]), 2))
            ...
            tx1, tx2 = tangle.transactions[tips_list[0]], tangle.transactions[tips_list[1]]
            if verify_signature(tx1.sender_public_key, tx1.signature, tx1.get_transaction_dict()) and verify_signature(tx2.sender_public_key, tx2.signature, tx2.get_transaction_dict()):
                return tips_list
            
            else:
                print("Invalid Transaction Encountered")
                self.find_tips(algo)
...
if __name__=="__main__":
    parser = ArgumentParser(description="Run tangle with selected tip selection algorithm")
    parser.add_argument("-a","--algo", type=str, default="random", help="Choose a tip selection algorithm", choices=["weighted_random_walk", "unweighted_random_walk", "recently_added", "random"])
    args = parser.parse_args()
    tangle = Tangle()
    generate_transactions(initial= True, size=5)
    generate_transactions(size=40, tip_selection_algo=args.algo)
